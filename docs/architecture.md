...
**为什么不用 Server Actions 做主流程？** Server Actions 不天然支持多路并行流式，自己拼麻烦；走 Route Handler + AI SDK `streamObject` / `streamText` 更直接。

### 2. API 层 (统一 Node Runtime)
...
自托管 Docker 模式下**所有路由统一跑 Node Runtime**，不再使用 Vercel Edge。`/api/analyze` 因 pdf-parse 需 Node API、其他路由也用同一 runtime 简化构建。

**Body 大小限制：** `/api/upload` 显式设 `bodyParser: { sizeLimit: '5mb' }` 或在 route segment config 里 `export const maxDuration` / `export const fetchCache = 'force-no-store'`；Caddy 端默认 10MB 限制即可。

**速率限制分流：**
- `/api/analyze`（写：耗模型 token）：10 次/小时/IP，超限返回 429
- `/api/result/[id]`（读：仅查库）：不限或宽松到 60 次/分钟/IP，避免面试官打开链接被误伤
...
sequenceDiagram
    ...
    Note over O,MS: 第二层 (单独)
    O->>MS: score(jd_struct, resume_struct)
    MS-->>O: scores
    O-->>C: SSE: stage="phase2_done"

    Note over O,IV: 第三层 (并行, Rewriter 依赖 scores 做预筛)
    par
        O->>RW: rewrite(jd_struct, resume_struct, target_bullet_ids)
        O->>IV: interview(jd_struct, resume_struct, scores)
    end
    RW-->>O: rewrites
    IV-->>O: questions
...
- 面试时要能逐行讲清编排逻辑，自写更可控
- AI SDK 已经处理了流式 + 工具调用 + JSON schema 三件最难的事

**Demo 安全模式 (`?demo=1`)：**
- 落地页"立即试用"按钮默认带 `?demo=1`
- 编排器在 demo 模式下**不调用真实 LLM**，直接返回 `fixtures/golden-result.json`（一份事先用 Claude 生成并冻结的高质量样本结果），叠加伪流式延时（每个 Agent 1-2 秒）模拟真实分析体感
- 用途：面试现场零事故 + 零延迟 + 零模型费用，且**永远不会因为 NIM 限流而翻车**
- 真实分析仍可通过去掉 `?demo=1` 或显式 `?demo=0` 触发
...
- Llama (NIM) 模式：**不支持 caching**；NIM 个人免费额度**不限制总量，仅限制请求频率（RPM）**，对 V1 demo 期足够覆盖（编排器单请求 ≤5 个并行调用，远低于个人 RPM 上限，实测未触限）。如未来要支撑多人并发演示再考虑付费或自托管。
...
type AnalysisContext = {
  schema_version: string  // '1.0.0', 严格匹配 SCHEMA_VERSION
  ...
  input: {
    ...
    is_demo: boolean       // demo 模式 (?demo=1) 短路 LLM 调用
  }
...
**域名与 TLS：**
- DNS 用 Cloudflare（**DNS-only 模式，必须关闭橙云代理 / Proxy=Off**，否则 SSE 长连接会被 Cloudflare 截断并缓冲，"四面板流式跳字"演示直接失效）
...
| 全部 5 Agent 完成 (三阶段 DAG) | ≤ 18s (P50)，≤ 28s (P95) |
| 一次完整分析 API 成本 — Llama (NIM) 模式 | **$0**（NIM 个人版无总量限制，仅 RPM 限制）|
...
每次改动任何 Agent 的 prompt 或 schema，都必须能跑一次自动回归。`scripts/eval.ts` 设计：

- 输入：`fixtures/eval-cases/*.json` (每条含 `{jd_text, resume_text, expected: {agent_name: { must_contain_keys, value_ranges }}}`)
- 流程：对每个 case，调全套 Agent；用 Zod 校验 schema；用 case 内"必含字段/取值范围"做轻量断言
- 输出：通过率、平均耗时、token 成本、退化 diff
- Week 1 用 fixtures 内的 demo-jd/demo-resume 做基线 case；后续每改 prompt 都跑一次
...
| 维度 | 措施 |
|---|---|
| 简历内容传输 | HTTPS only；不写入服务端日志。`pino` 配置 `redact: { paths: ['*.resume_text', '*.jd_text', 'req.body.resume_text', 'req.body.jd_text'], censor: '[REDACTED]' }` |
...
| 密钥管理 | `NVIDIA_API_KEY` / `ANTHROPIC_API_KEY` / `POSTGRES_PASSWORD` 通过服务器 `.env` (chmod 600) 注入容器；不进仓库 |
...
| 框架 | next | ^15 |
| UI | react | ^19 |
| 样式 | tailwindcss | **^3.4**（v4 生态尚不稳，shadcn 主线仍是 v3） |
...
| PDF | **unpdf** (首选) / pdf-parse (备选) | latest — pdf-parse 维护停滞，优先用现代 unpdf |
| DB Client | postgres (porsager/postgres) | ^3 |
| Cache/RateLimit | ioredis | ^5 |
| Logger | pino + pino-pretty | latest (含 redact 配置) |
| ID | nanoid | ^5 |
...
- [ ] Llama 流式行为最终确认（双通道 narration+JSON 是否落地）—— **见 W1-Spike**
- [x] ~~NIM 真实免费额度核实~~ **已确认 2026-05-28：个人版无总量限制，只有 RPM 限制；当前 demo 体量足够覆盖**
